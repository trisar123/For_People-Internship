...
public class DrawLine : MonoBehaviour {
	...
	void Update() {
		// Create new line on right mouse is depressed
		...
		// Line renderer exists and right mouse button is released (exited)
		if(Input.GetMouseButtonUp(1) && line) {
			mousePos = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Camera.main.farClipPlane));
			mousePos.z = 0;
		...
		if(Input.GetMouseButton(1) && line) {
			mousePos = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Camera.main.farClipPlane));
			mousePos.z = 0;
